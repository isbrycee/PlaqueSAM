...
class BatchedVideoDatapoint:
    ...
    image_classify: list # add by bryce
    box_mask_pairs: list # add by bryce
...
def collate_fn(
    ...
    img_batch = []
    for video in batch:
        img_batch += [torch.stack([frame.data for frame in video.frames], dim=0)] # bs=1. (num_frames_per_video(8), 3, 1024, 1024)
    
    img_batch = torch.stack(img_batch, dim=0).permute((1, 0, 2, 3, 4)) # bs=1. (8, 1, 3, 1024, 1024) # note 1 is the batchsize of video
    T = img_batch.shape[0]
    ...
    step_t_box_mask_pairs = [] # add by bryce
    ...
    for video_idx, video in enumerate(batch):
        ...
        for t, frame in enumerate(video.frames):
            ...
            # for classes
            merged_meta_class_info =  list(meta_box_info.keys())
            ...
            # for box-mask pairs (e.g., single box prompt with its corresponding mask)
            step_t_box_mask_pairs.append(frame.box_mask_pairs)
            # end; add by bryce
            for obj in objects:
    ...
    obj_to_frame_idx = torch.stack(
        [
            torch.stack(obj_to_frame_idx, dim=0)[0,:].unsqueeze(0) # !!!! note here !!!! 
            for obj_to_frame_idx in step_t_obj_to_frame_idx
        ],
        dim=0,
    )
