...
def flip_rects_left_right(rects, width):
    if isinstance(rects, tuple):
        rects = rects[0]
    flipped_rects = {}
    for name, points in rects.items():
        flipped_points = flip_points_left_right(points, width)
        flipped_rects[name] = flipped_points
    return flipped_rects

def resize_points(points, orig_width, orig_height, new_width, new_height):
    # 计算宽度和高度的比例因子
...
def resize_rects(rects, orig_width, orig_height, new_width, new_height):
    if isinstance(rects, tuple):
        rects = rects[0]
    resized_rects = {}
    for name, points in rects.items():
        resized_points = resize_points(points, orig_width, orig_height, new_width, new_height)
...
def hflip(datapoint, index):
    ...
    image_width = datapoint.size[1]
    datapoint.frames[index].boxes = flip_rects_left_right(datapoint.frames[index].boxes, image_width) # add by bryce
